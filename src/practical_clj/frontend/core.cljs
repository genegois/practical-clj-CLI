...
(defn register! [email password name] 
  ...
  (ajax/POST (str api-base "/sql/register")
    {:params {:email email :password password :name name}
     ...
     :error-handler #(swap! app-state assoc :error (or (get-in % [:response :body :message]) (get-in % [:response :message])))}))
...
(defn login! [email password]
  ...
  (ajax/POST (str api-base "/sql/login")
    {:params {:email email :password password}
     ...
     :error-handler #(swap! app-state assoc :error (str "Login gagal: " (or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
...
(defn fetch-profile! [] 
  ...
  (ajax/GET (str api-base "/sql/profile")
    {:headers (auth-header)
     ...
     :error-handler #(swap! app-state assoc :error (str "Gagal ambil profile: " (or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
...
(defn logout! [] 
  ...
  (ajax/POST (str api-base "/sql/logout")
    {:params {:refresh_token (:refresh-token @app-state)}
     ...
     :keywords? true
     :handler (fn [_] 
                (reset! app-state {:token nil
                                   :refresh-token nil
                                   :profile nil
                                   :orders nil
                                   :total-spent 0}))
     :error-handler #(swap! app-state assoc :error (str "Gagal logout: " (or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
...
(defn delete-account! []
  ...
  (ajax/DELETE (str api-base "/sql/delete-account")
    {:headers (auth-header)
     ...
     :error-handler #(swap! app-state assoc :error (str "Gagal hapus akun: "(or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
    ...
    {:headers (auth-header)
     ...
     :handler (fn [{:keys [orders total-spent]}]
                (let [email (get-in @app-state [:profile :email])
                  ...
                  (fetch-profile!)))
     :error-handler #(js/alert (str "Gagal load orders: " (or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
    ...
    {:headers (auth-header)
     ...
     :handler (fn [_] 
		(js/alert "Order berhasil dibuat!")
                (load-order!))
     :error-handler #(js/alert (str "Gagal bikin order: " (or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
  ...
  (ajax/PATCH (str api-base "/mongo/orders/" id "/status")
    {:headers (auth-header)
     ...
     :handler (fn [_] 
                ...
                (load-order!))
     :error-handler #(js/alert (str "Gagal update status order: " (or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
...
(defn delete-order! [id] 
  ...
  (ajax/DELETE (str api-base "/mongo/orders/" id)
    {:headers (auth-header)
     ...
     :error-handler #(js/alert (str "Gagal hapus order: "(or (get-in % [:response :body :message]) (get-in % [:response :message]))))}))
