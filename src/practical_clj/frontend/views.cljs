    ...
    (fn []
      [:div
       [:div {:style {:display "flex" :gap "8px" :margin-bottom "8px"}}
        ...
        [:button {:on-click load-orders!} "Tampilkan order"]
        [:button {:on-click logout!} "Logout"]
        [:button {:on-click delete-account!} "Hapus akun lo"]]
...
(defn create-order-form [create-order!]
  (let [product (r/atom "")
    ...
    (fn []
      [:div
       [:h3 "Buat Order"]
       [:input {:type "text" :placeholder "Produk" :value @product
                :on-change #(reset! product (.. % -target -value))}]
       [:input {:type "number" :placeholder "Harga" :value @price
                :on-change #(reset! price (.. % -target -value))}]
       [:select {:value @method :on-change #(reset! method (.. % -target -value))}
        [:option {:value "Cash"} "Cash"]
        [:option {:value "Transfer"} "Transfer"]]
       [:button {:on-click #(create-order! @product @price @method)}
        "Buat Order!"]])))
