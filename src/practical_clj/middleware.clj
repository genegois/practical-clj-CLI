  ...
  (fn [request]
    (let [auth-header (get-in request [:headers "authorization"])
      ...
      (cond
        ...
        :else
        (error 401 "unauthorized" "Token salah type")))))

(def custom-exception-middleware
  (exception/create-exception-middleware
   (merge exception/default-handlers
          {::exception/wrap (fn [_ _ _]
                              {:status 400 :body {:status "error" :message "Input tidak valid"}})})))
...
(defn make-rate-limiter [{:keys [limit window-ms] :or {limit 5, window-ms 6000}}]
  (let [store (atom (cache/ttl-cache-factory {} :ttl window-ms))]
    (fn [handler]
      (fn [req]
        (let [ip (or (get-in req [:headers "x-forwarded-for"])
                     (:remote-addr req) "unknown")
